 ...
 if(((LPC_TIM_TypeDef *) ((0x40000000UL) + 0x08000) )->IR & 1){ //MR0
  if(contatore>0){
   if(contatore==momentoTimer){
    ...
    contatorePalline=0;
    for(i=0;i<32 && found==-1;i++){
    for(j=0;j<24 && found==-1;j++){
     if(matrice[i][j]==1){
      contatorePalline++;
   ...
   if(contatore==9){
    for(i=0;i<16;i++){
     for(j=0;j<8;j++){
      LCD_SetPoint(j+218,i+10,0x0000);
     }
    }
   }
   GUI_Text(210,10,(uint8_t *)tempo,0xFFFF,0x0000);
  }else{
   //disable_timer(0);
   disable_timer(1);
   disable_RIT();
