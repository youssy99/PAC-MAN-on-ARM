...
void cambiaStato(int tipo,uint16_t colore){
 ...
 if(firstTime==0){
  for(i=0;i<50;i++){
  for(j=0;j<70;j++){
   LCD_SetPoint(j+90,i+140,0x0000);
  }
  }
 }
 if(tipo==-1){
  if(firstTime==0){
   GUI_Text(105,145,(uint8_t *)"Game",colore,0x0000);
   GUI_Text(105,160,(uint8_t *)"Over!",colore,0x0000);
   firstTime=1;
  }
  ...
  if(firstTime==0){
   GUI_Text(95,155,(uint8_t *)"Victory!",colore,0x0000);
   firstTime=1;
  }
  ...
  GUI_Text(105,155,(uint8_t *)"Pause",colore,0x0000);
 }
 return;
}
